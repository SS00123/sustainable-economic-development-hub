...
with main_col:
    ...
    with col_f2:
        quarter = st.selectbox(
            "Quarter", [1, 2, 3, 4], index=st.session_state.quarter - 1, key="filter_quarter"
        )
    with col_f3:
        regions = [
            "all",
        ...
        region = st.selectbox(
            "Region", regions, index=regions.index(st.session_state.region), key="filter_region"
        )
    with col_f4:
    ...
    try:
        ...
        with card_container(
            "Performance Overview", f"Q{quarter} {year} • {len(metrics)} KPIs tracked"
        ):
        ...
        with r1c2:
            ...
            if len(regional_df) > 0:
                ...
                regional_agg = regional_agg.sort_values(
        ...
        with r2c1:
            ...
            if breakdown:
                labels = [
                    item.get("name", item.get("name_en", "Unknown")) for item in breakdown[:6]
                ]
