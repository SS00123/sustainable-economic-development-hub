...
class TestJWTTokenExpiration:
    ...
    def test_token_with_custom_expiration(self, jwt_handler, sample_user):
        ...
        token = jwt_handler.create_access_token(sample_user, expires_delta=timedelta(minutes=5))
...
class TestMockUsers:
    ...
    def test_mock_users_have_required_fields(self):
        ...
        for _user_id, user in mock_users.items():
            assert isinstance(user, User)
...
class TestAPIAuthenticationFlow:
    ...
    def test_protected_endpoint_with_mock_user(self, client):
        ...
        response = client.get(
            "/api/v1/indicators", headers={"X-User-ID": "demo", "X-Tenant-ID": "test"}
        )
    ...
    def test_bearer_token_authentication(self, client):
        ...
        response = client.get(
            "/api/v1/sustainability/summary", headers={"Authorization": f"Bearer {token}"}
        )
...
class TestProductionSecurityEnforcement:
    ...
    def test_production_requires_jwt_secret(self):
        ...
        from analytics_hub_platform.infrastructure.settings import ConfigurationError, Settings
