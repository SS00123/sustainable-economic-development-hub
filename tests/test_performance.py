...
class TestDataFramePerformance:
    ...
    def test_groupby_aggregation_performance(self, large_dataframe):
        ...
        assert elapsed < FAST_OPERATION_THRESHOLD, f"Groupby took {elapsed:.3f}s, should be < {FAST_OPERATION_THRESHOLD}s"
...
class TestServicePerformance:
    ...
    def test_data_quality_metrics_performance(self, sample_data):
        ...
        start = time.perf_counter()
        quality = get_data_quality_metrics(sample_data, filters)
        elapsed = time.perf_counter() - start
