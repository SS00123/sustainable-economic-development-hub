...
class KPIForecaster:
    ...
    def _create_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        features["rolling_std_4"] = features["value"].rolling(window=4, min_periods=1).std().fillna(0)
    ...
    def _get_feature_columns(self) -> List[str]:
        return [
            "time_idx", "quarter_sin", "quarter_cos", "year_norm",
            "lag_1", "lag_2", "lag_4", 
...
class AnomalyDetector:
    ...
    def detect_isolation_forest_anomalies(
        ...
        settings = get_settings()
        contamination_val = contamination if contamination is not None else settings.anomaly_if_contamination
