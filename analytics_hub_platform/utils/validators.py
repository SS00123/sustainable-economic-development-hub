...
from analytics_hub_platform.domain.models import FilterParams, IndicatorRecord
...
def validate_indicator_data(
    ...
    if "data_quality_score" in data.columns:
        ...
        if len(low_quality) > 0:
            result.add_warning(
                f"{len(low_quality)} records have low data quality score (<50)"
            )
...
def validate_filter_params(
    ...
    if params.region is not None:
        if params.region not in REGIONS:
    ...
    if strict and not result.is_valid:
        raise ValidationError(
            f"Filter validation failed: {'; '.join(result.errors)}"
        )
