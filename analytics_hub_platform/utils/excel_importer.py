...
class ExcelCSVImporter:
    ...
    REQUIRED_COLUMNS = ["tenant_id", "kpi_id", "year", "quarter", "region", "value"]

    OPTIONAL_COLUMNS = ["unit", "target_value", "data_quality_score", "source", "notes"]
    ...
    def load_file(
        ...
        if isinstance(file_path_or_buffer, (str, Path)):
            ...
            if file_path.suffix.lower() in [".xlsx", ".xls"]:
                ...
                raise ValueError(
                    f"Unsupported file format: {file_path.suffix}. Use .xlsx, .xls, or .csv"
                )
        else:
    ...
    def validate_columns(
        ...
        required_cols = {col.strip().lower() for col in required}
