...
def render_saudi_map(
    ...
    for region_id, region_info in SAUDI_REGIONS.items():
        ...
        size_scale = 0.8 + 0.4 * ((value - min_val) / (max_val - min_val) if max_val != min_val else 0.5)
    ...
    if show_labels:
        fig.add_trace(go.Scattergeo(
            ...
            lat=lats,
            mode="text",
            text=[f"<b>{n}</b><br><span style='font-size:9px'>{v:.1f}</span>" for n, v in zip(names, values)],
            textfont=dict(
                size=10,
