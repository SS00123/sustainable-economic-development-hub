...
def render_data_quality_view() -> None:
    ...
    if missing_by_kpi:
        ...
        fig.add_vline(
            x=90,
            line_dash="dash",
            line_color=theme.colors.status_green,
    ...
    quality_trend = (
        df.groupby(["year", "quarter"]).agg({"data_quality_score": "mean"}).reset_index()
    )
    quality_trend["period"] = quality_trend.apply(
    ...
    fig2.add_hline(
        y=80, line_dash="dash", line_color=theme.colors.status_green, annotation_text="Target: 80"
    )
