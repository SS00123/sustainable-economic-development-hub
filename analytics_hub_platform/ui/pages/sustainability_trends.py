...
def render_sustainability_trends() -> None:
    ...
    render_header(
        ...
        subtitle=strings.get(
            "sustainability_subtitle", "Environmental and Green Economy Indicators"
        ),
    ...
    trend_df = df.groupby(["year", "quarter"]).agg(dict.fromkeys(env_kpis, "mean")).reset_index()
    ...
    sus_trend = df.groupby(["year", "quarter"]).agg({"sustainability_index": "mean"}).reset_index()
    sus_trend["period"] = sus_trend.apply(
    ...
    fig2.add_trace(
        go.Scatter(
            ...
            line={"color": theme.colors.success, "width": 3},
            marker={"size": 10},
            fill="tozeroy",
    ...
    fig2.add_hline(
        y=70,
        line_dash="dash",
        line_color=theme.colors.status_amber,
