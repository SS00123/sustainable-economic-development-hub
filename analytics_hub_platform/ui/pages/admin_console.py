...
def render_kpi_management(theme) -> None:
    ...
    selected_kpi = st.selectbox(
        ...
        format_func=lambda x: next((k.get("display_name", {}).get("en", x) for k in kpis if k.get("id") == x), x),
    )
...
def render_tenant_info(theme) -> None:
    ...
    st.info("💡 Multi-tenant support ready. Production deployment will connect to organization directory.")
