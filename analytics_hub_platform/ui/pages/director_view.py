...
from analytics_hub_platform.ui.filters import get_filter_state, render_period_comparison_filters
...
def render_overview_tab(snapshot: dict, sustainability: dict, theme, filters) -> None:
    ...
    with col_right:
        ...
        st.markdown(f"**Region:** {filters.region.title() if filters.region != 'all' else 'All Regions'}")
...
def render_trends_tab(df: pd.DataFrame, theme, filters) -> None:
    ...
    with col2:
        ...
        trend_agg = trend_df.groupby(["year", "quarter"]).agg({
            selected_kpi: "mean"
        }).reset_index()
    ...
    if compare_kpis:
        compare_df = trend_df.groupby(["year", "quarter"]).agg({
            k: "mean" for k in compare_kpis
        }).reset_index()
        ...
        fig = go.Figure()
        colors = [theme.colors.primary, theme.colors.secondary, theme.colors.success, theme.colors.warning]
...
def render_regional_tab(df: pd.DataFrame, theme, filters) -> None:
    ...
    # Aggregate by region
    regional_agg = regional_df.groupby("region").agg({
        selected_kpi: "mean"
    }).reset_index()
...
def render_sustainability_tab(sustainability: dict, df: pd.DataFrame, theme, filters) -> None:
    ...
    with col1:
        ...
        colors = [
            theme.colors.status_green if s == "green"
            else theme.colors.status_amber if s == "amber"
            else theme.colors.status_red if s == "red"
    ...
    with col2:
        ...
        for item in breakdown:
            ...
            status_emoji = "🟢" if status == "green" else "🟡" if status == "amber" else "🔴" if status == "red" else "⚪"
