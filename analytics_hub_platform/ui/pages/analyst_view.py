...
def render_analyst_view() -> None:
    ...
    tab1, tab2, tab3 = st.tabs(["📋 Data Explorer", "✅ Data Quality", "📤 Export Center"])
...
def render_data_explorer_tab(df: pd.DataFrame, theme, filters) -> None:
    ...
    with col4:
        ...
        display_columns = [
            c for c in all_columns if c not in ["id", "tenant_id", "source_system", "load_batch_id"]
        ]
    ...
    with col1:
        ...
        if st.button("📥 Generate Excel Export", type="primary"):
            try:
                ...
                render_alert_box(
                    f"Rate limit exceeded. Please try again in {e.retry_after} seconds.", "warning"
                )
            except Exception as e:
    ...
    with col2:
        ...
        if st.button("📥 Generate CSV Export"):
            try:
                ...
                csv_df = df[(df["year"] == csv_filter_year) & (df["quarter"] == csv_filter_quarter)]
