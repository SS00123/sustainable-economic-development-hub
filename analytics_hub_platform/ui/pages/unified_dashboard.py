...
def render_unified_dashboard() -> None:
    ...
    with main_col:
        ...
        with st.container():
            ...
            with col_f1:
                new_year = st.selectbox("Year", [2024, 2023, 2022, 2021], 
                    index=[2024, 2023, 2022, 2021].index(year), key="filter_year")
            with col_f2:
                new_quarter = st.selectbox("Quarter", [1, 2, 3, 4], 
                    index=quarter - 1, key="filter_quarter")
            with col_f3:
                ...
                new_region = st.selectbox("Region", regions, 
                    index=regions.index(region) if region in regions else 0, key="filter_region")
            with col_f4:
                new_language = st.selectbox("Language", ["en", "ar"], 
                    index=["en", "ar"].index(language), key="filter_language")

            # Update session state if changed
            if new_year != year or new_quarter != quarter or new_region != region or new_language != language:
        ...
        # =========================================================================
        st.markdown("<div id='section-overview' style='height: 16px;'></div>", unsafe_allow_html=True)
        ...
        # =========================================================================
        render_dark_section_title("🌱 Sustainability Index", "Composite score for sustainable development")
        ...
        render_dark_section_title("📊 Key Performance Indicators", f"Core metrics for Q{quarter} {year}")
        
        # KPI subgrids by category
        _render_dark_subgrid("Economic Performance", [
            "gdp_growth", "gdp_total", "foreign_investment", "export_diversity_index", "economic_complexity",
        ], metrics, dark_theme)

        _render_dark_subgrid("Labor & Skills", [
            "unemployment_rate", "green_jobs", "skills_gap_index", "population",
        ], metrics, dark_theme, columns=4)

        _render_dark_subgrid("Social & Digital", [
            "social_progress_score", "digital_readiness", "innovation_index",
        ], metrics, dark_theme, columns=3)

        _render_dark_subgrid("Environmental & Sustainability", [
        ...
        with trend_col1:
            ...
            fig.add_trace(go.Scatter(
                ...
                line=dict(color=dark_theme.colors.purple, width=3),
                marker=dict(size=10, color=dark_theme.colors.purple),
                fill='tozeroy',
            ...
            if len(trend_agg) > 2:
                ...
                fig.add_trace(go.Scatter(
                    ...
                    mode="lines",
                    name="Trend",
                    line=dict(color=dark_theme.colors.cyan, width=2, dash="dash"),
                ))
        ...
        if len(regional_df) > 0:
            ...
            with reg_col1:
                ...
                colors = [dark_theme.colors.purple if v >= national_avg else dark_theme.colors.pink for v in regional_agg["sustainability_index"]]
                ...
                fig.add_vline(x=national_avg, line_dash="dash", line_color=dark_theme.colors.cyan, 
                             annotation_text=f"Avg: {national_avg:.1f}", annotation_font=dict(color="#94a3b8"))
                
                apply_dark_chart_layout(fig, height=400)
                fig.update_layout(
                    xaxis=dict(showgrid=True, gridcolor='rgba(255,255,255,0.05)', title="Sustainability Index", 
            ...
            with reg_col2:
                ...
                for label, value in [("National Average", f"{national_avg:.1f}"), 
        ...
        # =========================================================================
        st.markdown("<div id='section-environment' style='height: 8px;'></div>", unsafe_allow_html=True)
        render_dark_section_title("🌿 Sustainability & Environmental Trends", "Multi-indicator performance")
        st.markdown("<div style='height: 12px;'></div>", unsafe_allow_html=True)
        ...
        trend_env = df.groupby(["year", "quarter"]).agg({k: "mean" for k in env_kpis if k in df.columns}).reset_index()
        trend_env = add_period_column(trend_env)  # Vectorized period creation
        trend_env = trend_env.sort_values(["year", "quarter"])

        fig_env = go.Figure()
        palette = [
            dark_theme.colors.purple, dark_theme.colors.cyan, dark_theme.colors.pink,
            "#f472b6", "#818cf8", "#4ade80", "#fbbf24", "#fb923c",
        ]
        label_map = {
            "co2_index": "CO2 Intensity",
        ...
        # =========================================================================
        st.markdown("<div id='section-data-quality' style='height: 8px;'></div>", unsafe_allow_html=True)
        ...
        for col, (label_text, value_text) in zip([dq_col1, dq_col2, dq_col3, dq_col4], 
...
def _render_activity_chart(df: pd.DataFrame, year: int, quarter: int, region: str, dark_theme) -> None:
    ...
    fig.add_trace(go.Scatter(
        ...
        y=trend_agg["sustainability_index"],
        mode="markers",
        marker=dict(size=8, color=dark_theme.colors.purple, 
...
def _render_dark_subgrid(title: str, kpis: list, metrics: dict, dark_theme, columns: int = 4) -> None:
    ...
    for i, kpi_id in enumerate(kpis):
        ...
        with cols[i % columns]:
            ...
            # Status colors
            status_bg = "#22c55e22" if status == "green" else "#f59e0b22" if status == "amber" else "#ef444422" if status == "red" else "#6b728022"
...
def _render_subgrid(title: str, kpis: list, metrics: dict, theme, columns: int = 4) -> None:
    ...
    st.markdown(f"<h3 style='margin: 0 0 16px 0; color: #0f172a; font-size: 15px; font-weight: 600;'>{title}</h3>", unsafe_allow_html=True)
    cols = st.columns(columns)
...
def _render_ml_forecast_section(df: pd.DataFrame, year: int, quarter: int, theme, language: str) -> None:
    ...
    try:
        ...
        forecast_kpis = ["sustainability_index", "gdp_growth", "non_oil_gdp_share", "unemployment_rate"]
        ...
        if len(hist_df) < 8:
            st.warning("⚠️ Not enough historical data for reliable forecasting (need at least 8 quarters).")
            return
        ...
        fig.update_layout(
            ...
            xaxis=dict(showgrid=True, gridcolor='rgba(0,0,0,0.05)', title="Period"),
            yaxis=dict(showgrid=True, gridcolor='rgba(0,0,0,0.05)', title=selected_kpi.replace("_", " ").title()),
            legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1),
...
def _render_anomaly_section(df: pd.DataFrame, year: int, quarter: int, theme, language: str) -> None:
    ...
    try:
        ...
        if not all_anomalies:
            ...
            for anomaly in sorted(all_anomalies, key=lambda x: (x.severity.value, -x.year, -x.quarter))[:10]:
                severity_color = theme.colors.status_red if anomaly.severity == AnomalySeverity.CRITICAL else theme.colors.status_amber
...
def _render_llm_recommendations_section(snapshot: dict, theme, language: str) -> None:
    ...
    _render_section_title("🤖 AI Strategic Recommendations", "LLM-powered insights aligned with Vision 2030")
    ...
    if "ai_recommendations" in st.session_state and st.session_state["ai_recommendations"]:
        ...
        # Provider info
        st.caption(f"Generated by {result.get('provider', 'AI')} ({result.get('model', 'N/A')}) at {result.get('generated_at', 'N/A')}")


def _render_regional_map_section(df: pd.DataFrame, year: int, quarter: int, theme, language: str) -> None:
    """Render the regional map section."""
    import streamlit.components.v1 as components
    
    _render_section_title("🗺️ Regional Performance Map", "Interactive visualization of KPIs across Saudi Arabia")
