...
from analytics_hub_platform.utils.kpi_utils import get_delta_suffix, get_kpi_unit
...
def render_executive_view() -> None:
    ...
    render_section_header(
        ...
        description=strings.get(
            "sustainability_desc", "Composite sustainability and development score"
        ),
        icon="🌱",
    )
    ...
    # Key KPIs Grid
    render_section_header(title=strings.get("key_kpis", "Key Performance Indicators"), icon="📊")
    ...
    for col, kpi_id in zip(cols1, row1_kpis, strict=False):
        with col:
    ...
    for col, kpi_id in zip(cols2, row2_kpis, strict=False):
        with col:
            kpi = metrics.get(kpi_id, {})
            kpi_value = kpi.get("value")
            if kpi_value is None:
