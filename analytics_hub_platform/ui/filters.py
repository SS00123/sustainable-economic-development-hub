...
def render_period_comparison_filters() -> tuple:
    ...
    with col2:
        ...
        if compare_type == "prev_quarter":
            ...
            comp_year = st.number_input("Year", value=current_year - 1, min_value=2020, max_value=2024)
            comp_quarter = st.selectbox("Quarter", options=[1, 2, 3, 4], format_func=lambda x: f"Q{x}")
